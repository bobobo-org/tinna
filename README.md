...
- 目前功能：接下來 7 天的預約總覽、預約列表（日期／狀態篩選、顧客資料與問題、下載 CSV）、VIP 會員與方案、訂單、商品與運費、KOL 推薦碼。
...
**VIP 包堂（網站 `/vip`、後台「VIP」「訂單」）**
- 客人在 `/vip` 選方案（預設 4／10／12／15／20 堂，每堂 90 分鐘，有效 365 天）→ 信用卡付款 → 付款成功自動發 VIP 卡號（`VIP-XXXX-XXXX`），寄到購買時的 Email，付款完成頁也會顯示。
- 預約時選「VIP 諮詢」（`services` 的 `vip`，`vip_only = true`，不會出現在一般方案列表），輸入 VIP 卡號＋購買時的 Email，每次扣 1 堂、直接確認時段，不需付款。`/vip` 下方可以查詢剩餘堂數與預約紀錄。
- 後台「VIP」：會員查詢（姓名／Email／卡號／手機）、「本月壽星」（寄生日禮用，可下載 CSV）、調整堂數／期限／備註（例：退還一堂、延長期限）；方案的價格、堂數、有效天數、上下架都可以改（只影響之後的購買）。
- 後台「訂單」：VIP 購買（之後的商店、VIP 贈品也在這裡）的付款狀態、顧客資料；取消訂單不會自動退款，要另外到綠界後台辦理。

**開運選物商店與 VIP 贈品（網站 `/shop`、後台「商品」「訂單」「VIP」）**
- 後台「商品」：新增商品（名稱、價格、庫存、說明、圖片最多 8 張，上傳前自動縮小）。勾「在商店販售」才會出現在 `/shop`；不勾就是「只當贈品」（可以是 0 元）；「啟用」不勾＝下架。下方可以設定宅配運費（預設 NT$100）與滿額免運。
- 客人在 `/shop` 把商品加入購物車 → 填訂購與收件資料（可用推薦碼，只折商品不折運費）→ 信用卡付款 → 付款成功才扣庫存，寄訂單確認信；後台「訂單」填物流單號標記出貨，會寄出貨通知信。
- 寄 VIP 贈品／生日禮：後台「VIP」→ 會員「詳細／調整」→「寄贈品／生日禮」→ 選商品與數量、填收件地址 → 建立贈品訂單（不收費、立即扣庫存）→ 到「訂單」頁出貨。
- 商品圖片存在 Supabase Storage 的公開 bucket `products`（第一次上傳時自動建立）。
...
- 新增一支 migration：`supabase/migrations/<序號>_<名稱>.sql`，序號是 4 位數遞增（目前到 `0008_shop.sql`，下一支會是 `0009_xxx.sql`；已套用到正式庫的檔案不要再改）。
