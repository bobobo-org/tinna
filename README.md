...
- 目前功能：接下來 7 天的預約總覽、預約列表（日期／狀態篩選、顧客資料與問題、下載 CSV）、VIP 會員與方案、訂單、KOL 推薦碼。
...
- 新增一支 migration：`supabase/migrations/<序號>_<名稱>.sql`，序號是 4 位數遞增（目前到 `0007_commerce.sql`，下一支會是 `0008_xxx.sql`；已套用到正式庫的檔案不要再改）。
