...
import { breadcrumbLd, teacherLd } from '@/lib/structured-data';
...
export default function AboutPage() {
  return (
    <>
      <JsonLd data={teacherLd(DESCRIPTION)} />
      <JsonLd
        data={breadcrumbLd([
          { name: '首頁', path: '/' },
          { name: '老師介紹', path: '/about' },
        ])}
      />
      ...
      <section className="bg-page px-[clamp(24px,4vw,56px)] py-[72px]">
        <div className="mx-auto grid max-w-[1100px] grid-cols-[repeat(auto-fit,minmax(min(100%,300px),1fr))] gap-12">
          ...
          <div data-reveal style={{ '--reveal-delay': '150ms' } as React.CSSProperties} className="flex flex-col gap-5">
            <h2 className="font-serif text-[28px] font-bold text-ink-900">諮詢原則</h2>
            {PRINCIPLES.map((p) => (
              <div key={p.title} className="lift rounded-2xl border border-rose-600/[.18] bg-white p-6">
