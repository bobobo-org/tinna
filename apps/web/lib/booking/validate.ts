 ...
 *
 * - Step 1：要選方案；Step 2：要選日期與時段（這兩步只決定「下一步」能不能按，不顯示訊息）
 * - Step 3：姓名非空、出生日期必填、Email、手機數字 ≥ 9 碼
...
export function validateDetails(f: BookingForm, today: string): FieldErrors {
  const e: FieldErrors = {};
...
export function validateStep(step: Step, s: ValidationInput, today: string): FieldErrors {
  const e: FieldErrors = {};
  if (step === 1 && !s.svc) e.svc = MSG.svc;
  if (step === 2 && !(s.date && s.time)) e.dt = MSG.dt;
  if (step === 3) Object.assign(e, validateDetails(s.f, today));
  if (step === 4) {
...
export function firstIncompleteStep(s: ValidationInput, today: string): Step {
  ...
  if (Object.keys(validateDetails(s.f, today)).length > 0) return 3;
  return 4;
}
