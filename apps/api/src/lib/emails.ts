import type { BookingFull, PayMethod } from '../db/types';
...
export function confirmationEmail(b: BookingFull, meetUrl: string | null): MailMessage {
  ...
  const html = layout(
    '預約成功',
    `<p style="font-size:15px;line-height:2">${esc(b.customerName)} 您好，感謝你的預約，付款已完成。以下是這次諮詢的資訊：</p>
${rows(items)}
